...
export async function getTileFromDb(tilePath: string): Promise<{ data: Buffer; format: number } | null> {
...
export async function getSettingsFromDb(key: string): Promise<string | null> {
...
export async function getStandaloneFileFromDb(fileName: string): Promise<string | null> {
...
export async function getUserFromDb(username: string): Promise<{ password_hash: string; salt: string } | null> {
...
export async function registerUserInDb(username: string, passwordHash: string, salt: string): Promise<boolean> {
	const tableNames = [`${prefix}users`, `${prefix}Users`, `users`, `Users`];
	for (const table of tableNames) {
...
export async function getRegistrationCode(code: string): Promise<string | null> {
...
export async function getMarkerFileFromDb(markerName: string): Promise<string | null> {
...
export async function getMarkerIconFromDb(iconParam: string): Promise<Buffer | null> {
...
export async function getPlayerFaceFromDb(faceParam: string): Promise<Buffer | null> {
	if (!faceParam.startsWith('faces/')) return null;
...
export async function writeStandaloneFileToDb(fileName: string, content: string): Promise<boolean> {
	const tableNames = [`${prefix}StandaloneFiles`, `${prefix}standalonefiles`, `StandaloneFiles`, `standalonefiles`];
	for (const table of tableNames) {
